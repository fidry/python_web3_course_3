...
from data.config import SYNCSWAP_ROUTER_ABI


class SyncSwap:
    def __init__(self, client: Client):
        self.client = client
        self.router_abi = read_json(SYNCSWAP_ROUTER_ABI)
        ...
        steps = [
            [
                ...
                self.client.w3.to_hex(
                    abi.encode(
                        ...
                        [from_token_contract.address, zk_weth_c_lp, 2]
                    )
                ),
                zero_address,
                '0x',
            ...
            [
                ...
                self.client.w3.to_hex(
                    abi.encode(
                        ['address', 'address', 'uint8'],
                        [zk, self.client.account.address, 1]
                    )
                ),
                zero_address,
                '0x',
